"""Batch state management for resumable F1 ETL processing."""
...
class BatchStateManager:
    """Manages F1 ETL batch processing state for resumable operations."""
    
    def __init__(self, state_file: str = "f1_etl_state.json"):
    ...
    def start_processing(self, total_rows: int):
        ...
        self.logger.info(f"Started F1 ETL processing: {total_rows:,} rows, batch size {settings.BATCH_SIZE:,}")
    
    def save_progress(self, batch_number: int, end_row: int, records_processed: int, records_skipped: int = 0):
        ...
        Args:
            ...
            records_skipped: Number of records skipped due to validation failures
        """
        ...
        self.state["records_skipped"] += records_skipped
        ...
        self.logger.info(
            ...
            f"- {records_processed:,} processed, {records_skipped:,} skipped"
        )
    ...
    def mark_complete(self):
        ...
        self.logger.info(
            f"F1 ETL run completed successfully: {self.state['batches_completed']} batches, "
            f"{self.state['records_processed']:,} records processed, "
            f"{self.state['records_skipped']:,} records skipped"
        )
    ...
    def mark_failed(self, error_message: str):
        ...
        self.logger.error(f"F1 ETL run marked as failed: {error_message}")
    ...
    def clear_state(self):
        ...
        self.logger.info("Clearing F1 ETL state - will start from beginning")
    ...
    def get_state_info(self) -> Dict:
        ...
        state_info = {
            ...
            "Records Skipped": f"{self.state.get('records_skipped', 0):,}",
