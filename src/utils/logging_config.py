"""Logging configuration for the F1 Qualifying ETL Pipeline."""
...
def create_run_log_file() -> str:
    ...
    log_filename = f"f1_etl_run_{timestamp}.log"
...
def setup_logging(
    ...
    """Set up logging configuration for the F1 ETL pipeline.
    ...
    # Create logger
    logger = logging.getLogger('f1_etl_pipeline')
...
def get_logger(name: str) -> logging.Logger:
    ...
    """
    return logging.getLogger(f'f1_etl_pipeline.{name}')


class ETLProgressLogger:
    """Helper class for logging F1 ETL progress and statistics."""
    ...
    def log_resume_info(self, resume_batch: int, resume_row: int, reason: str = ""):
        ...
        """
        message = f"Resuming F1 ETL processing from batch {resume_batch}, row {resume_row:,}"
        if reason:
            message += f" ({reason})"
